...
| Ψ₄ | Weyl4WithMatter: E/B decomposition (Alcubierre) with the gr-qc/0104063 tetrad; every stream stores rΨ₄ projected on spin −2 harmonics, Re/Im | amplitudes quoted as rΨ₄; in-code and consumer agree to 0.1 % | `Source/CCZ4/Weyl4.hpp`; paper §III.C |
...
| flow finder | shape-free MOTS hunt `grteclyn-wrapper/scripts/validation/ah_flow_finder.py`: r = h(θ,φ) in real Y_lm to ℓ = 6 (or 8), damped fast flow, areal orientation. Validated on Schwarzschild (R = 2M to 0.03 %), Ellis (no surface), head-on remnant (2 %) | script docstring; paper §III.C |
...
| campaign pin | `runs/wormhole_merger/bin/main3d_boost_2026-09-08.ex`, the `launch.sh` default binary (ignores the l2 seed and the core profile; the preflight refuses templates that use them) | `launch.sh`; `results/merger/binaries.tsv` |
