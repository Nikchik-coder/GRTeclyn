...
row to §2 and, if it settles something, one line to §4; the numbers themselves are
...
it is `GPU_PLAN.md` (frozen 2026-09-03); the campaign to 2026-09-02 is `Plan.md`.
...
## 2. Where we stand (2026-09-09)

| phase | state | evidence |
|---|---|---|
| Stage 0 — is a lone throat stable? | **Done: no.** Exact static data holds 26 units, then the Gonzalez–Guzman–Sarbach radial mode grows at τ = 5.9 (T = τ_proper/r_throat 0.87 against the predicted 0.68–0.76). The binary "wall" at t = 44–56 was never a binary effect. | README "The initial data is exact, and the throat is unstable anyway"; `single_throat/INSTABILITY.md` |
| Phase 2 — resolution ladder, χ twins, time-step bracket | **Done.** Level 2 dies at the origin at t = 24.2 with the throat exact (grid, not clamp: its χ twin dies the same). Level 3 **collapses**: marginally trapped surface at t ≈ 61, R 3.23 → 2.37 by t = 100, Misner–Sharp mass inside 1.62 → 1.23, no bounce by t = 100. Level 4 **inflates**: R 3.89 → 10.0, no horizon, growing anti-trapped shell, compactified inner sheet deforming by t = 100. Same rate to 9 % (τ 5.88 vs 5.26), onset +5.6 to +8.7 units per halving (seed of order 1.5–2.3, sign flips). χ floor not load-bearing at levels 2, 3, 4 (twins to 4 digits; level-4 pair byte-identical). dt_multiplier 0.02 necessary (0.05 is a different solution from t ≈ 33, 0.1 dies at t = 16). Late constraint growth from t ≈ 75 on both branches, unexplained. | `single_throat/BRANCHES.md`, `figures/single_throat_branches.png`, `runs_registry.tsv`; archive "Ladder result" I and II, "Time-step bracket" |
| Phase 1 — code | **Coded, built, smoke-tested, not yet in a physics run:** point-of-use χ regularisation (`chi_rhs_floor`), solution-following tagger, det h̃ rescale, shock-avoiding lapse, ε seed per throat with sign, boosted Π. **In use:** corrected θ± shell scan with Misner–Sharp mass, extremum guard and depth column (tested on Schwarzschild and the Ellis throat), NaN autopsy. **GRTresna:** phantom sign and drainhole profile coded, one solve done — throat present, ψ +2–5 % high from the outer boundary condition on ψ_reg (1 where the exact answer is 1 − b/2r); bridge untested. **Not done:** W = √χ, MOTS stability eigenvalue, the boundary condition. | archive "Phase 1" table; `consume_plotfiles/README.md`; GRTresna `feature/grteclyn-wrapper` 5bfa159 |
| Phase 3 — V1, head-on from rest | Not started. Gate: the pair must touch before the lone throat departs — from d = 8 contact ≈ 17 by the calibrated law, against a 0.1 % departure at t = 35 (level 3) / 41 (level 4). | archive "Phase 3" |
| Phase 4 — V2, production with extraction | Not started. | archive "Phase 4" |

Cards: all four free. Nothing in flight. Scratch: 670 GB free (the ladder's kept
plotfiles were pruned 2026-09-09 on the user's word). Frames: every launch now
renders χ, K, lapse, φ, Π by default (launcher, 2026-09-09) — the ladder runs
rendered χ only, and no other movie of the collapse or the inflation can be made.

## 3. Next — the queue

| # | what | cards × wall | decides | success reads |
|---|---|---|---|---|
| 1 | **±ε arms at level 3**: seed ±1e-3, ±1e-2, ±1e-1 on the areal-radius function (`wormhole_seed_amplitude_A`), six runs from `single_hold_t100`'s template, frozen `bin/main3d_boost_2026-09-08.ex`, frames χ K lapse φ Π, plotfiles kept-last for the θ± scan, no checkpoints | 6 × ~5 h on 4 cards | The branch by choice instead of by grid noise: τ and the horizon time per branch; the onset against ln ε (a straight line of slope −τ says the seed is the only thing that moved); whether −ε reproduces level 4's inflation *with* the inner-sheet deformation (physics) or without it (the origin failing); the ε at which noise stops mattering, which sets the seed V1 must declare. | +ε: collapse, MOTS, mass loss as at level 3. −ε: inflation, anti-trapped shell, no MOTS. Onset(ε) linear in ln ε with slope ≈ −5.9. |
| 2 | GRTresna outer boundary condition on ψ_reg → 1 − b/2r; re-solve `params_drainhole_test.txt`; push the solution through `ExternalGridInitialData` | code, no GPU | Constraint-solved data for V1/V2 (the +8–11 % superposition error, or the seed, is otherwise in every claim) | solved ψ within 1 % of the exact drainhole at N = 64, throat at R = 2.0; the bridged run holds the throat to t = 20 |
| 3 | V1 scout: head-on from rest, d = 8, level 3, seed declared, several frame fields, θ± scan on | 1 × ~5 h | Do the throats meet as wormholes before their own decay, and does the merged core collapse (2 → 1 surfaces) or something else? | contact by t ≈ 17 with both throats within 0.1 % of exact; surface count 2 → 1 by the θ± scan |
