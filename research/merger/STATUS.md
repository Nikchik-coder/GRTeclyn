...
**Plan vs paper**: the paper is the current word — 959 ledger rows, 0 problems (`claims.py check`, re-run 2026-09-25 afternoon after Fig. 11's vacuum-control overlays) — EXCEPT the 09-25 η = 4 horizon results and the spherical-literature inflation ending (Shinkai–Hayward / GGS II: exponential in proper time), which are in the plan and registry only. The plan's older entries still carry superseded readings (the regrowth as physics, "no horizon ever forms" for the spiral, "+17 %" regrowth, the ×7.8 fly-by growth as a measurement, the "21 % short" ringdown).
