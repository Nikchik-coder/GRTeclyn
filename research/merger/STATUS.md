...
|---|---|---|---|---|
| `single_pureq_q1e2_L128_ml4_scalar_t500` | first GPU node, GPU 0 | t = 115.2 / 500 | t = 150 in 5.1 h (09:35 today); t = 300 Fri 07:30; t = 500 Sat 09-26 ~12:45 | where the inflating throat ends: saturation, coast or turnover. Read at t = 100–112: "RUN ON" |

- That arm is **an unkicked L = 128 level-4 throat**, not a pure quadrupole: it was
  launched on the campaign pin, which does not read `wormhole_seed_l2_amplitude_A`
...
- **t250 TAKE 2** `single_eps_p1e2_L128_ml4_t250`: L = 128, level 4, ε = +10⁻², checkpoints on; ~37 h. Card: first-node GPU 1.
...
- Audit runs A1–C2 below. A1/A2 need a seeded binary: `main3d_guard_7166787a_2026-09-24.ex` (stamped, reads the seed and the core profile).
...
- **Waves**: every channel radiates; the fly-by is loudest; the scalar channel is comparable to the gravitational one (ratio 3.2 → 0.8 with the cut) and negative-energy; the horizon switches it off. (§VIII)
...
- AMReX ignores keys nothing reads: old binaries run new params without the new
  physics. The launch preflight now refuses this; say `--binary` explicitly.
