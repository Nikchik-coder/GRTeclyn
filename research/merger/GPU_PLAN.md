...
Cards: all four free (the scout died 03:04; the placement probes used card 1 02:06–02:48). Scratch: ~750 GB free (everything pruned
...
| ☐ | 1c | **V1 scout with the interior freeze** — the scout's template, plus the stage-3 chain's interior fill/freeze switched on once the common horizon exists (t ≈ 22; freeze radius inside the fine-scan MOTS r ≈ 3, e.g. r = 1.5, sparing the trapped shells' outer part), checkpoints on from t = 20 for the phase-2 refinement | 1 × ~10 h | Is the wall an interior artefact the horizon makes irrelevant? If the freeze carries the run to t = 100 the outside (horizon area, M_MS, ψ4) is the result | no NaN to t = 100; horizon persists or dissolves *smoothly*; constraints bounded outside r ≈ 3 |
