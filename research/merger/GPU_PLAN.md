...
- [ ] **Phase 3 — V1, head-on from rest, d = 8** → archive "Phase 3"
  ...
  - [ ] lone throat at m = 0.5 (queue 1g): the control that dates the 1d throats' own decay — **do not read 1d's fate without it**
...
| ☐ next free card | 1g | **lone throat at m = 0.5 — the control for 1d**: `single_hold_t100`'s template with `wormhole_drainhole_mass_A` 1.0 → 0.5, everything else as the low-mass head-on (level 3, chi_rhs_floor, NaN autopsy, frames χ K lapse φ Π, areal radius, no checkpoints), stop_time 40 is enough for the onset | 1 × ~3.5 h | The instability clock at half the mass (onset 26, e-fold 5.9 are m = 1 numbers; the rate depends on m/a). Without it a throat in 1d that starts changing at t ≈ 20 cannot be attributed to the contact rather than to its own decay | onset and e-fold time at m = 0.5; whichever branch it takes, the 1d mouths are read against it |
