...
- [ ] **Phase 3 — V1, head-on from rest, d = 8** → archive "Phase 3"
  ...
  - [ ] the interior freeze carries the merger past the wall (queue 1c, **in flight since 2026-09-09 03:24**) — and the two Weyl4 streams (in-code at R = 10/14/18 and the consumer's ψ4 at the same radii) agree
...
Cards: 0 busy (the V1b freeze arm since 03:24), 1–3 free. Scratch: ~750 GB free (everything pruned
...
| 🚀 in flight 2026-09-09 03:24, card 0 | 1c | **V1b freeze arm** — `merge_headon_flip_d8_v1b_freeze_t100` (template `templates_scan/params_v1b_freeze_d8_t100.txt`, launcher `launch_v1_freeze.sh`, same frozen binary): the scout byte-for-byte plus (1) the smooth interior fill `core_freeze_fill` at t = 22.5, r_full 1.2 / r_start 2.0 — inside the trapped surface (r 3.17 at t = 22), outside the failure site (r 0.89); (2) in-code Weyl4 extraction at **R = 10, 14, 18**, 21 modes, every coarse step, with the python consumer's ψ4 at the same three radii as the independent stream; (3) Weyl4_Re frames over a 40-wide window with χ K lapse φ Π; (4) checkpoints every 5 units, stop_time 100. ~11.1 units/h: freeze ≈ 05:30, t = 100 ≈ 12:30 | 1 × ~9 h | Is the wall an interior artefact that the horizon makes irrelevant? If the freeze carries the run to t = 100 the *outside* (horizon area, M_MS, the two ψ4 streams) is the result | no NaN to t = 100; the horizon persists or dissolves *smoothly* across the engagement; the two Weyl4 streams agree; constraints bounded outside r ≈ 3 |
