...
4b. **And a single throat, alone, does the same thing.** `single_hold_t100` (2026-09-05):
   ...
   **Both fates, both amplitudes.** The collapse arms form black holes: a horizon from **t = 11**
   ...
   expands to R ≈ 9 at r ≈ 9.5. Figure: `plot_seed_branches` (retired from figures/ 2026-09-26, not in the paper; the paper's Fig. 1(c) draws the same panel).
   ...
   **Four follow-up arms, stopped by hand on 2026-09-10 with the cards wanted elsewhere, and what
   ...
   4/5 through t ≈ 28 — and then t ≳ 45, where the burst has passed all four spheres. **The rerun
   was done** (queue 2e, 2026-09-14/15): level 4 with rolling checkpoints carried the ε = 0.01 arm to
   t = 51 in two legs, and a fresh arm at five times the quadrupole reached t = 100 and answered all
   five gates — see "One throat radiates — if its collapse is not spherical". (c) *The Ψ4 floor rise at t ≈ 40 is not the outer boundary.*
   ...
   *(pack, `seed/`)*, stopped by hand at t = 195.14 on 2026-09-24, matches its L = 64 twin to
   ...
   measured (`plot_single_inflation_L128`, retired from figures/ 2026-09-26, not in the paper; registry row). The kicked
   ...
   (`plot_single_inflation_L512`; the paper's `figures/01_single_throat/single_throat_inflation`; registry row).
...
### One throat radiates — if its collapse is not spherical

- **Claim (measured).** Break the spherical symmetry of a collapsing drainhole
  and it emits a gravitational wave. An l = 2 shell of amplitude ε₂ = 0.05 laid
  on the +0.01 spherical kick (`single_eps_p1e2_q5e2_ml4_t100` *(pack,
  `campaign/01_single_throat/seed/`)*, max_level 4, L = 64, N = 128, t = 100,
  spheres R = 10 / 14 / 18 / 22) produces a (2,0) burst of **peak |r·Ψ₄| =
  2.9e-3** carrying **E_rad = 2.3e-6 M**, arriving at t = 13 / 18 / 22 / 27 —
  in radius order, at 0.8–1.0 c — and ringing afterwards at a single frequency,
  **f = 0.0485/M, period 20.6 M**. This is the first gravitational wave in the
  campaign that does not need two objects. Figure:
  the `plot_psi4_analysis` page of `single_eps_p1e2_q5e2_ml4_t100` (retired from figures/ 2026-09-26, not in the paper).
- **The five gates, decided before the runs were launched**, are measured in
  full in [`campaign/01_single_throat/QUEUE2E_GATES.md`](campaign/01_single_throat/QUEUE2E_GATES.md)
  (regenerated by `analysis/queue2e_gates.py` on every repack). (1) *Ordered
  arrival* — passes; the 0.8 c readings are the 1-unit output cadence against
  ΔR = 4 quantising the lag to whole samples, not a subluminal wave. (2)
  *r·Ψ₄ equal across the spheres* — 2.87–3.08e-3 over the common retarded
  window u = 0–39, a **7.2 %** spread with no radial trend (the largest peak is
  at the *outermost* sphere): a pass at ten per cent, not at the few per cent
  the gate asked for. (3) *Amplitude tracks ε₂* — against the ε₂ = 0.01 arm
  ...
  window, the signal 35× above it.
- **The late-time excursions are numerical, and the control proves it.** From
  t ≈ 60 every sphere picks up swings three to ten times the wave, near
  simultaneous at all radii. The spherical control `single_eps_p1e2_t100` —
  same kick, **no quadrupole at all** — carries the same growth at R = 14
  (3.3e-5 through t = 40, 2.9e-3 by t = 70, 1.8e-1 past t = 80) while its
  R = 30 sphere never leaves 1.1e-3. A run with no quadrupole cannot radiate a
  quadrupole, so this is sphere-local numerical growth. Every window above is
  closed before it starts (per-sphere cuts t = 70 / 69 / 63 / 61 at
  R = 10 / 14 / 18 / 22), and the figure is drawn on the gated record
  (`scripts/plot/gate_psi4_junk.py`, cosine taper *ending* at the cut). With the
  junk left in, the wavefront matcher reads **v = 4 c** — that is how loud it is.
  Low-pass filtering was tried first and rejected: 12–38 % distortion of the
  clean wave at every cutoff and no reduction at all at R = 18, the junk having
  in-band power.
- **Against the mergers.** This is a small wave. Peak |r·Ψ₄| 2.9e-3 against the
  head-on's 2.3e-2 and the d = 12 spiral's 1.8e-2 — **6 to 8× weaker** — and
...
- **Caveats.** (a) The ε₂ = 0.01 arm reaches only t = 51 and is stitched from two
  ...
  *frequency* is amplitude-independent — its per-sphere frequencies scatter
  ...
  prediction — this hole dissolves into the phantom field rather than settling
  to Schwarzschild, and M_MS = 1.56 is itself a scan number. (d) Nothing is
  quoted past t = 80 (the constraint onset in this family) or outside R ≤ 22
  (the sponge's inner edge is 24). (e) **This run has no usable movies** — the
  consumer was given `--frames-axis y` without `--frames-center`, the renderer
  centred the window at z = 0 and the throat at z = 32 was out of frame for the
  whole run; unrecoverable, since the slice cache stored only the cropped
  window and the plotfiles are gone. See the run's `LOST.md`, and rule 13 in
  the wrapper README. The physics is unaffected: it is measured from the Ψ4
  streams, not the pictures.
...
- **The head-on freeze arms (2026-09-09).** `merge_headon_flip_d8_v1c_latefreeze_t100`
  *(pack)* — the scout with the fill (r_full 1.2 / r_start 1.8) armed at t = 26.5,
  ...
  (the down-step's `psi4` page, retired from figures/ 2026-09-26, not in the paper; both no-fill arms reached
  ...
  the freeze arm's `psi4` page (retired from figures/ 2026-09-26, not in the paper).
...
- **Runs.** Eighteen probes `place_d{6,65,7,75,8,10,12,14,16,18,20,24,28,32,36,40,44,48}_step1`
  *(pack)* — initial data plus one step each, scanned at
  ...
  rows.
- **Where.** `campaign/04_binary_headon/PLACEMENT_CURVE.md` (tables, generated), `plot_placement_curve` (retired from figures/ 2026-09-26, not in the paper; the paper's Fig. 4(d,e) draws its panels),
