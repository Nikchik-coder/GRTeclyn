...
control) across all three panels.  The gold accent is the reference the
...
def main(argv: list[str] | None = None) -> int:
    ...
    axB.loglog(f_hz_band, np.sqrt(_aLIGO_noise_psd(f_hz_band)),
               color=style.GOLD, linewidth=1.2, zorder=2,
               label="aLIGO design")
    ...
    axC.plot(-tau_an, newtonian_chirp_M(tau_an) * to_hz, color=style.GOLD,
    ...
    axC.text(-43.0, 68.0, "point-mass\nchirp", fontsize=6.5, ha="left",
             va="bottom", color=style.GOLD, linespacing=1.2)
    axC.set_xlim(-45, 30)
