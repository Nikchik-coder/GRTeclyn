...
def areal(run_dir: pathlib.Path) -> np.ndarray | None:
    for rel in ("small_data/areal_radius.dat", "areal_radius.dat"):
        ...
        if f.is_file():
            ...
            return a[np.argsort(a[:, 0])][:, :2] if a.size else None
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name, label in wanted:
        ...
        m = dict(name=name, label=label, t=a[:, 0], R=a[:, 1], dead=died(d))
        m["dev"] = m["R"] - args.exact
        m["cross"] = zero_crossing(m["t"], m["dev"])
        m["rate"] = rate(m["t"], m["dev"], args.window)
        arms.append(m)
        print(f"  {label:>7s}  {name:<24s} t = 0 .. {a[-1, 0]:6.2f}"
              f"{'   (NaN)' if m['dead'] else ''}")
    if not arms:
    ...
    for m in arms:
        ...
        axA.plot(m["t"], m["R"], color=c, linewidth=1.6, zorder=3)
    ...
    for m in arms:
        ...
        ok = d > 0
        axB.semilogy(m["t"][ok], d[ok], color=c, linewidth=1.6, zorder=3)
