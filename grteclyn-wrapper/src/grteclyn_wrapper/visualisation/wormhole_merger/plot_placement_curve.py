...
scout's mouths are the thing being calibrated (gold, unjoined, because they
...
Monochrome ink plus the one accent: GOLD is the scout (its readings in
...
def figure_panels(ax1, ax2, pack_root=PACK_ROOT, stacked: bool = True) -> None:
    ...
    if len(scout):
        # Same in/out encoding as panel (b): gold where a probe backs the
        ...
        ax1.plot(scout[inside, 1], scout[inside, 2], ls="none", marker="s",
                 ms=3.2, color=style.GOLD, zorder=5)
        ax1.plot(scout[~inside, 1], scout[~inside, 2], ls="none", marker="s",
    ...
    if stacked:
        ...
        if len(scout):
            ax1.text(6.9, 4.415, "scout", fontsize=8, color=style.GOLD,
                     ha="center", va="top")
            if (~inside).any():
    ...
    if len(scout):
        ...
        ax2.plot(t[inside], resp[inside], marker="s", ms=3.2, zorder=5,
                 color=style.GOLD, linestyle=(0, ()), linewidth=1.2)
        ax2.set_xlabel(r"$t$")
