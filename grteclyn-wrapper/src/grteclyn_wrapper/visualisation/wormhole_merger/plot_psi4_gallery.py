...
keys, and after 2026-09-16 review NO TEXT INSIDE THE PANELS AT ALL beyond the
cap notes -- the scenario, its knob, the mode/spheres and the speeds all sit
on the strip above each row's frames, where no waveform can collide with
them -- monochrome ink with the grey ramp inner->outer on the envelopes, the
gold accent reserved for the wavefront, deep blue for the one fitted model,
(a)-(e) tags, semantics in the caption.  7.05 x 5.6 in since 2026-09-24
(was 7.05 x 7.0).
"""
    ...
    for i, r in enumerate(rows):
        ...
        if u_end < 0.97 * xhi:
            ...
            if r["note"]:
                ...
                axL.text(u_end + 0.012 * (xhi - xlo), 0.12 * pk, r["note"],
