#!/usr/bin/env bash
# Launch one matrix cell for fgeo_max_cmaes_v2 (RM / RC / RF / …).
#
#   bash scripts/campaigns/promote/fgeo_max_cmaes_v2/run.sh --list
#   DRY_RUN=1 bash scripts/campaigns/promote/fgeo_max_cmaes_v2/run.sh FMAX-RM
#   GPU_ID=2 bash scripts/campaigns/promote/fgeo_max_cmaes_v2/run.sh FMAX-RI
set -euo pipefail
