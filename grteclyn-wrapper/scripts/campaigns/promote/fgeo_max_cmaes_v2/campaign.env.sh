...
# (FMAX-*). v2 clone of fgeo_max_cmaes_v1, repointed at the Phase-3 freeze
# (eval_000187, score 4169.2). Cloned per GPU_RUN_PLAN.md §12.2 with the
...
export CAMPAIGN_NAME="fgeo_max_cmaes_v2"
...
export LIVE_RUN="${LIVE_RUN:-${GRTECLYN_ROOT}/runs/neuralspacetime/search/cma_es/qball_traj_fgeo_max_cmaes_v2}"
export FREEZE_ROOT="${FREEZE_ROOT:-${GRTECLYN_ROOT}/runs/neuralspacetime/hq/sources/qball_traj_fgeo_max_cmaes_v2}"
...
# Solve: the v1 single-rank pin is GONE — the segfault it worked around
# belonged to the old node; MPI has been healthy since 2026-08-19 and Phase 3
# ran 200 evals as three concurrent 8-rank solves (one unrelated crash).
#
# Set to 8 EXPLICITLY rather than deleted, contra GPU_RUN_PLAN.md §12.2's
# wording: hq/run_batch.sh runs under `set -euo pipefail` and expands
# "${GRTRESNA_RANKS}" with no `:-` fallback (unlike EVOLUTION_MPI_RANKS one
# line above), so an unset value aborts the cell with `unbound variable` the
# moment it reaches the solve. replay_eval.py's argparse default of 8 never
# applies because run_batch always passes the flag. Verified 2026-08-28.
#
# Gate at the 5% the search accepted the champion under, not the promote
# default 10% (§12.2).
